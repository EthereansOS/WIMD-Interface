...
const buldJsxs = async function(replacer, basePath, path) {
    ...
    for (let entry of fs.readdirSync(basePath + path)) {
        ...
        if ((fs.lstatSync(basePath + curPath)).isDirectory()) {
            ...
            if (!entry.endsWith('.min.css') && !entry.endsWith('.jsx') && !entry.endsWith('controller.js')) {
                continue;
            }
            ...
            source = entry.endsWith('controller.js') ? source : babel.transform(source.split('<>').join('<React.Fragment>').split('</>').join('</React.Fragment>').split('this.useState').join('useState').split('useState(').join('useState(this, ').split('this.useEffect').join('useEffect').split('useEffect(').join('useEffect(this, '), babelArgs).code;
            source = entry.endsWith('controller.js') ? source : uglify.convenience(source);
